...
class Ground extends React.Component {
    ...
    handleBattle() {
        ...
        for (var i = 0; i < playersArray.length; i++) {
            ...
            } else if (playersArray[i].name === this.state.players[i].name && playersArray[i].card[0].life <= 0) {
                //alert(`Put a card ${this.state.players[i].name}`)
            }
    ...
    getTurnAttack(player, attack, array) {
        ...
        for (var i = 0; i < array.length; i++) {
            ...
            if (player !== array[i].name && array[i].card[0].life >= 0 && array[i].card[0].defense >= 0 && attack[1] !== array[i].card[0].weakness) {
                ...
                // return a percentage of attack and defense
                 attackWPercent = Math.round((attack[0] * attackPercent) / 100);
                 defWPercent = Math.round((attack[0] * defPercent) / 100);


                // soustraction for life and defense
                 lifeCard = Math.round(array[i].card[0].life - (attackWPercent * 1.5));
                 defenseCard = Math.round(array[i].card[0].defense - (defWPercent * 1.2));
                ...
                var attackNoDefPercent = Math.floor(Math.random() * (60 - 15)) + 15;
                 lifeCard = array[i].card[0].life - Math.round((attack[0] * attackNoDefPercent) / 100);
                ...
                attackNoDefPercent = Math.floor(Math.random() * (60 - 15)) + 15;
                 lifeCard = array[i].card[0].life - Math.round((attack[0] * attackNoDefPercent) / 100);
